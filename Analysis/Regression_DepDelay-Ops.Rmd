...
flights_2018_DD <- flights_2018 %>%
  ...
  filter(Dep_Delay > 0) %>%  #changed per y variable
  group_by(Origin) %>% 
...
Plotting mean departure delay versus total operations by airport. 
```{r}
flights_2018_DD %>%
...
Linear regression of mean departure delay on total operations by airport suggests a slightly decreasing departure delay as total operations increases. 
```{r}
lm_Dep_delay <- lm(Mean_Dep_Delay ~ Tot_Ops.x, data = flights_2018_DD)
...
After logging x, the plot of mean departure delay on total operations now looks like:   
```{r}
flights_2018_DD %>%
  ggplot(aes(x = log2(Tot_Ops.x), y = Mean_Dep_Delay)) +
  geom_point(na.rm = TRUE) +
  geom_smooth(method = "lm", se = FALSE, na.rm = TRUE) +
  xlab("Total Airport Operations") +
  ylab("Mean Departure Delay") +
...
```{r}
lm_Dep_delay <- lm(Mean_Dep_Delay ~ log2(Tot_Ops.x), data = flights_2018_DD)
lm_Dep_delay_tidy <- tidy(lm_Dep_delay)
lm_Dep_delay_tidy
lm_Dep_delay_augment <- augment(lm_Dep_delay)
```
...
```{r}
flights_2018_DD %>%
  ggplot(aes(x = log2(Tot_Ops.x), y = log(Mean_Dep_Delay))) +
  geom_point(na.rm = TRUE) +
  geom_smooth(method = "lm", se = FALSE, na.rm = TRUE) +
  xlab("Total Airport Operations") +
  ylab("Mean Departure Delay") +
...
The revised linear regression model is now: 
```{r}
lm_Dep_delay <- lm(log(Mean_Dep_Delay) ~ log2(Tot_Ops.x), data = flights_2018_DD)
lm_Dep_delay_tidy <- tidy(lm_Dep_delay)
lm_Dep_delay_tidy
lm_Dep_delay_augment <- augment(lm_Dep_delay)
```
