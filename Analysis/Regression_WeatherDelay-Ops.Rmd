---
title: "Regression: Weather_Delay on Total Ops"
...
flights_2018_WD <- flights_2018 %>%
  ...
  filter(Weather_Delay > 0) %>%  #changed per y variable
  group_by(Origin) %>% 
...
Plotting mean weather delay versus total operations by airport. 
```{r}
flights_2018_WD %>%
  ggplot(aes(x = Tot_Ops.x, y = Mean_Weather_Delay)) +
...
Linear regression of mean weather delay on total operations by airport suggests a slightly decreasing weather delay as total operations increases. 
```{r}
lm_Weather_delay <- lm(Mean_Weather_Delay ~ Tot_Ops.x, data = flights_2018_WD)
lm_Weather_delay_tidy <- tidy(lm_Weather_delay)
lm_Weather_delay_tidy
lm_Weather_delay_augment <- augment(lm_Weather_delay)
lm_Weather_delay_augment
```
...
After logging x, the plot of mean weather delay on total operations now looks like:   
```{r}
flights_2018_WD %>%
  ggplot(aes(x = log2(Tot_Ops.x), y = Mean_Weather_Delay)) +
  geom_point(na.rm = TRUE) +
  geom_smooth(method = "lm", se = FALSE, na.rm = TRUE) +
  xlab("Total Airport Operations") +
  ylab("Mean Weather Delay") +
...
```{r}
lm_Weather_delay <- lm(Mean_Weather_Delay ~ log2(Tot_Ops.x), data = flights_2018_WD)
lm_Weather_delay_tidy <- tidy(lm_Weather_delay)
lm_Weather_delay_tidy
lm_Weather_delay_augment <- augment(lm_Weather_delay)
```

And the revised plot of residuals shows variance is much more evenly distributed, although there seems to be a bit of skew for all x. 
```{r}
lm_Weather_delay_augment %>%
  ggplot(aes(x = .fitted, y = .std.resid)) +
  geom_point() +
...
```{r}
flights_2018_WD %>%
  ggplot(aes(x = log2(Tot_Ops.x), y = log2(Mean_Weather_Delay))) +
  geom_point(na.rm = TRUE) +
  geom_smooth(method = "lm", se = FALSE, na.rm = TRUE) +
  xlab("Total Airport Operations") +
  ylab("Mean Weather Delay") +
...
```{r}
lm_Weather_delay <- lm(log2(Mean_Weather_Delay) ~ log2(Tot_Ops.x), data = flights_2018_WD)
lm_Weather_delay_tidy <- tidy(lm_Weather_delay)
lm_Weather_delay_tidy
lm_Weather_delay_augment <- augment(lm_Weather_delay)
```
