---
title: "Regression: Carrier_Delay on Total Ops"
...
Plotting mean carrier delay versus total operations by airport. 
```{r}
flights_2018_CD %>%
  ggplot(aes(x = Tot_Ops.x, y = Mean_Carrier_Delay)) +
...
Linear regression of mean carrier delay on total operations by airport suggests a slightly decreasing carrier delay as total operations increases. 
```{r}
...
After logging x, the plot of mean carrier delay on total operations now looks like:   
```{r}
flights_2018_CD %>%
  ggplot(aes(x = log2(Tot_Ops.x), y = Mean_Carrier_Delay)) +
  geom_point(na.rm = TRUE) +
  geom_smooth(method = "lm", se = FALSE, na.rm = TRUE) +
  xlab("Total Airport Operations") +
  ylab("Mean Carrier Delay") +
...
```{r}
lm_Carrier_delay <- lm(Mean_Carrier_Delay ~ log2(Tot_Ops.x), data = flights_2018_CD)
lm_Carrier_delay_tidy <- tidy(lm_Carrier_delay)
lm_Carrier_delay_tidy
lm_Carrier_delay_augment <- augment(lm_Carrier_delay)
```
...
```{r}
flights_2018_CD %>%
  ggplot(aes(x = log2(Tot_Ops.x), y = log2(Mean_Carrier_Delay))) +
  geom_point(na.rm = TRUE) +
  geom_smooth(method = "lm", se = FALSE, na.rm = TRUE) +
  xlab("Total Airport Operations") +
  ylab("Mean Carrier Delay") +
...
The revised linear regression model is now: 
```{r}
lm_Carrier_delay <- lm(log2(Mean_Carrier_Delay) ~ log2(Tot_Ops.x), data = flights_2018_CD)
lm_Carrier_delay_tidy <- tidy(lm_Carrier_delay)
lm_Carrier_delay_tidy
lm_Carrier_delay_augment <- augment(lm_Carrier_delay)
```
